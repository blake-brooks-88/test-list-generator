...
} from "../hooks/useTestListConfig";

const renderUseProgress = () => {
  const wrapper = ({ children }) => (
    <TestListConfigProvider>{children}</TestListConfigProvider>
  );
...
function testVariantFieldBehavior(result) {
  ...
  const unselect = (field) =>
    act(() => result.current.unselectVariantField(field));
...
function testTestDataFieldBehavior(result) {
  ...
  const select = (field) =>
    act(() => result.current.selectTestDataField(field));
  const unselect = (field) =>
    act(() => result.current.unselectTestDataField(field));
...
describe("useTestListConfig", () => {
  ...
  test("setSelectedDE clears CSV data", () => {
    ...
    expect(result.current.varianceData).toEqual("");
